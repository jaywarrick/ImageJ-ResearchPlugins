...
public class AutoThresholder {
	...
	public int Intermodes(int[] data)
	{
		...
		while (!bimodalTest(iHisto))
		{
			...
			{
				threshold = -1;
				Logs.log("Intermodes Threshold not found after 10000 iterations.", 2, this);
				return threshold;
			}
		}
	...
	public int Li(int[] data)
	{
		...
		do
		{
			...
			if(temp < -2.220446049250313E-16)
				...
				new_thresh = (int) (temp + 0.5);
			/*
			 * Stop the iterations when the difference between the new and old threshold values is less than the tolerance
			 */
		}
	...
	public int MinErrorI(int[] data)
	{
		...
		while (threshold != Tprev)
		{
			...
			if(sqterm < 0)
			{
				Logs.log("MinError(I): not converging. Try Ignore black/white options", 2, this);
				return threshold;
			}
			...
			{
				Logs.log("MinError(I): NaN, not converging. Try Ignore black/white options", 2, this);
				threshold = Tprev;
			}
			else
	...
	public int Minimum(int[] data)
	{
		...
		while (!bimodalTest(iHisto))
		{
			...
			{
				threshold = -1;
				Logs.log("Minimum Threshold not found after 10000 iterations.", 2, this);
				return threshold;
			}
		}
	...
	public int Moments(int[] data)
	{
		...
		/*
		 * First 4 moments of the gray-level image should match the first 4 moments of the target binary image. This leads to 4 equalities whose solutions are given in the Appendix of Ref. 1
		 */
		cd = m0 * m2 - m1 * m1;
		c0 = (-m2 * m2 + m1 * m3) / cd;
		c1 = (m0 * -m3 + m2 * m1) / cd;
	...
	public double partialSum(int[] y, int j)
	{
		double x = 0;
		for (int i = 0; i <= j; i++)
			x += y[i];
