...
public class GaussianBlurForcedRadius implements ExtendedPlugInFilter, DialogListener {
    ...
    public void run(ImageProcessor ip) {
        ...
        blurGaussian(ip, sigmaX, sigmaY, Math.max(sigmaX, sigmaY)*3);
    }
    ...
    */
    ...
    public boolean blur(ImageProcessor ip, double radius) {
        ...
        blurGaussian(ip, 0.4*radius, 0.4*radius, 0.4*radius*3);
        return true;
    }
...
//	}
     ...
     * @param outerRadius Accuracy of kernel, should not be above 0.02. Better (lower)
     *    accuracy needs slightly more computing time.
     */
    public void blurGaussian(ImageProcessor ip, double sigmaX, double sigmaY, double outerRadius) {
        ...
        for (int i=0; i<ip.getNChannels(); i++) {
            ...
            blurFloat(fp, sigmaX, sigmaY, outerRadius);
     ...
     *                  accuracy needs slightly more computing time.
     */
    public void blurFloat(FloatProcessor ip, double sigmaX, double sigmaY, double outerRadius) {
        if (sigmaX > 0)
            blur1Direction(ip, sigmaX, outerRadius, true, (int)Math.ceil(5*sigmaY));
        if (Thread.currentThread().isInterrupted()) return; // interruption for new parameters during preview?
        if (sigmaY > 0)
            blur1Direction(ip, sigmaY, outerRadius, false, 0);
        return;
    }
     ...
     * @param outerRadius  Accuracy of kernel, should not be > 0.02
     ...
     */
    public void blur1Direction( final FloatProcessor ip, final double sigma, final double outerRadius,
        ...
        final boolean doDownscaling = false; //sigma > 2*MIN_DOWNSCALED_SIGMA + 0.5;
        ...
        final int maxLength = doDownscaling ?
                ...
                : length;
        final float[][] gaussKernel = makeGaussianKernel(sigmaGauss, outerRadius, maxLength);
                            ...
                            for (int line=lineFrom + ti; line<lineTo; line += numThreads, pixel0+=numThreads*lineInc) {
                                ...
                                if (doDownscaling) {
                                    ...
                                    for (int i=readFrom; i<readTo; i++ ,p+=pointInc)
                                        cache1[i] = pixels[p];
                                    convolveLine(cache1, cache2, gaussKernel, 0, newLength, 0, newLength, 0, 1);
                                    p = pixel0 + readFrom*pointInc;
                                    for (int i=readFrom; i<readTo; i++ ,p+=pointInc)
                                        pixels[p] = cache2[i];
                                }
     ...
     * @param outerRadius  Relative accuracy; for best results below 0.01 when processing
     ...
     */
    public float[][] makeGaussianKernel(final double sigma, final double outerRadius, int maxRadius) {
        int kRadius = (int) (Math.ceil(outerRadius)); //(int)Math.ceil(sigma*Math.sqrt(-2*Math.log(accuracy)))+1;
        ...
        {
        	float v = (float) Math.exp(-(i*i/(2*sigma*sigma)-i*i/(2*sigma*sigma)*(0.999*i*i/(30*2*sigma*sigma+i*i))));
//        	float v = (float) Math.exp(-(i*i/2*sigma*sigma));
//        	if(v < 1000*Float.MIN_VALUE)
//        	{
//        		v = 1000*Float.MIN_VALUE;
//        	}
        	kernel[0][i] = v;
        }
...
//            sum = sigma * Math.sqrt(2*Math.PI);
        ...
        double rsum = 0.5 + 0.5*kernel[0][0]/sum - 2*kernel[0][kRadius-1]/sum;
