...
public class JEXDialog {
	...
	{
		JEXDialog.parent = parent;
	}
	
	public static synchronized void messageDialog(String msg)
	{
		...
		Thread t = new Thread(new Runnable()
		{
			public void run()
			{
				JOptionPane.showMessageDialog(JEXDialog.parent, msg);
	...
	public static synchronized void messageDialog(String msg, Class<?> source)
	{
		Thread t = new Thread(new Runnable()
		{
			public void run()
	...
	public static synchronized void messageDialog(String msg, Object source)
	{
		Thread t = new Thread(new Runnable()
		{
			public void run()
	 ...
	 */
	public static synchronized Integer getChoice(String title, String question, String[] choices, int defaultChoice)
	{
		...
		int option = JOptionPane.showConfirmDialog(JEXDialog.parent, panel, title, JOptionPane.OK_CANCEL_OPTION);
		if(option == 0)
		{
	...
	public static synchronized String fileSaveDialog()
	{
		return fileDialog(true, false);
	}
	
	public static synchronized String fileChooseDialog(boolean directoriesOnly)
	{
		return fileDialog(false, directoriesOnly);
	}
	
	private static synchronized String fileDialog(boolean save, boolean directoriesOnly)
	{
...
//		String lastPath = PrefsUtility.getLastPath();
		...
		if(save)
		{
			returnVal = fc.showSaveDialog(JEXDialog.parent);
		}
		else
		{
			returnVal = fc.showOpenDialog(JEXDialog.parent);
		}
